...
calc_surv_aah <- nimble::nimbleFunction(
    ...
    #S0 <- nimArray(NA,c(2,nT_age,nT_period))
     S0 <- nimArray(0,c(2,n_agef,n_year))
    #UCH <- nimArray(NA,c(2,nT_age,nT_period))
    s_aah <- nimArray(0,c(2,n_agef,n_year))
		
# for age 1
        for(j in 1:n_year) {
			cll_uch <- beta0 + rep(age_effect[1:yr_end[1]],times=intvl_step_yr)+rep(period_effect[yr_start[j]:yr_end[j]],each=intvl_step_yr)
...
# for age 2+
    for(i in 2:n_agef) {
        for(j in 1:n_year) {
			cll_uch <- beta0 + rep(age_effect[1:yr_end[i]],times=yr_end[j])+rep(period_effect[1:yr_end[j]],each=yr_end[i])
...
calc_surv_harvest <- nimble::nimbleFunction(
    ...
    # s_aah[sex,age,period]
    S0 <- nimArray(0,c(2,n_agef,n_year))
    S0_hunt <- nimArray(0,c(2,n_agef,n_year))
    S0_hunt2 <- nimArray(0,c(2,n_agef,n_year))
    s_hunt <- nimArray(0,c(2,n_agef,n_year))
