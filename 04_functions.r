...
calc_surv_harvest <- nimble::nimbleFunction(
    ...
    # UCH_fawn <- nimArray(NA,c(2,intvl_step_yr,nT_period))
    # UCH_hunt_fawn <- nimArray(NA,c(2,intvl_step_yr,nT_period))
...
#     }
    ...
    for(i in 1:n_year){
        for(j in yr_start[i]:(ng_start[i]-1)){
            ...
            # UCH_hunt_fawn[1,1:intvl_step_yr,j] <- UCH_fawn[1,1:intvl_step_yr,j]
            # UCH_hunt_fawn[2,1:intvl_step_yr,j] <- UCH_fawn[2,1:intvl_step_yr,j]
        }
        ...
        for(j in ng_start[i]:(gun_start[i]-1)){
            ...
            # UCH_hunt_fawn[1,1:intvl_step_yr,j] <- UCH_fawn[1,1:intvl_step_yr,j] * p_nogun_f
            # UCH_hunt_fawn[2,1:intvl_step_yr,j] <- UCH_fawn[2,1:intvl_step_yr,j] * p_nogun_m
        }
        ...
        for(j in gun_start[i]:(gun_end[i])){
            ...
            # UCH_hunt_fawn[1,1:intvl_step_yr,j] <- UCH_fawn[1,1:intvl_step_yr,j] * p_gun_f
            # UCH_hunt_fawn[2,1:intvl_step_yr,j] <- UCH_fawn[2,1:intvl_step_yr,j] * p_gun_m
        }
    ...
    # s_aah[sex, age(years), period(years)]
    for (t in 1:n_year) {
        # s_hunt[1,1,t] <- exp(-sum(UCH_hunt_fawn[1,1:ng_end[1],yr_start[t]:ng_end[t]])) #antlerless fawns
        # s_hunt[2,1,t] <- exp(-sum(UCH_hunt_fawn[2,1:ng_end[1],yr_start[t]:ng_end[t]]))  #antlered fawns
        ...
        #s_hunt[1, 1, t] <- S0_hunt[1, ng_end[1], ng_end[t]] #antlerless fawns
        #s_hunt[2, 1, t] <- S0_hunt[2, ng_end[1], ng_end[t]] #antlered fawns
    }
    # first year across all ages from sex-specific process model
    # antlerless yearlings and older first year
    for(a in 2:n_agef) {
	...
	# age = 2 (denominator must come from fawns survival calculation)
			# wrong numerator
            #s_hunt[1, 2, t] <- S0_hunt[1, ng_end[2], ng_end[t]]/
            #                    s_hunt[1,1,t-1]
            #s_hunt[2, 2, t] <- S0_hunt[2, ng_end[2], ng_end[t]]/
            #                    s_hunt[2,1,t-1]
			# still wrong (need exp of hazard sums, not sum of exp hazards)
            #s_hunt[1, 2, t] <- (S0_hunt[1,ng_end[2],ng_end[t]] + S0[1,ng_end[1],ng_end[t-1]] - s_hunt[1,1,t-1])/s_hunt[1,1,t-1]
            #s_hunt[2, 2, t] <- (S0_hunt[2,ng_end[2],ng_end[t]] + S0[2,ng_end[1],ng_end[t-1]] - s_hunt[2,1,t-1])/s_hunt[2,1,t-1]
			# correct, I think (no, still wrong)
            #s_hunt[1,2,t] <- exp(log(S0_hunt[1,ng_end[2],ng_end[t]]) + log(S0[1,yr_end[1],yr_end[t-1]]) - log(S0_hunt[1,yr_end[1],yr_end[t-1]]))/exp(-sum(UCH_hunt_fawn[1,1:yr_end[1],yr_start[t]:yr_end[t]]))
            #s_hunt[2,2,t] <- exp(log(S0_hunt[2,ng_end[2],ng_end[t]]) + log(S0[2,yr_end[1],yr_end[t-1]]) - log(S0_hunt[2,yr_end[1],yr_end[t-1]]))/exp(-sum(UCH_hunt_fawn[2,1:yr_end[1],yr_start[t]:yr_end[t]]))
			
            #corrected version but still has uch_fawn, which  isn't necessary
            # s_hunt[1,2,t] <- exp(
			# log(S0_hunt[1,ng_end[2],ng_end[t]]) - 
			# sum(UCH_fawn[1,1:yr_end[1],yr_start[t-1]:yr_end[t-1]]) + 
			# sum(UCH_hunt_fawn[1,1:yr_end[1],yr_start[t-1]:yr_end[t-1]]))/
			# exp(-sum(UCH_hunt_fawn[1,1:yr_end[1],yr_start[t-1]:yr_end[t-1]]))
            # s_hunt[2,2,t] <- exp(
			# log(S0_hunt[2,ng_end[2],ng_end[t]]) - 
			# sum(UCH_fawn[2,1:yr_end[1],yr_start[t-1]:yr_end[t-1]]) + 
			# sum(UCH_hunt_fawn[2,1:yr_end[1],yr_start[t-1]:yr_end[t-1]]))/
			# exp(-sum(UCH_hunt_fawn[2,1:yr_end[1],yr_start[t-1]:yr_end[t-1]]))
			
            #corrected version but w/ *_fawn, because it wasn't necessary
            s_hunt[1,2,t] <- exp(
			log(S0_hunt[1,ng_end[2],ng_end[t]]) - 
    ...
    for (a in 3:n_agef) {
            ...
            #                    S0[1, yr_end[a-1], yr_end[t - 1]]
            s_hunt[1,a,t] <- exp(log(S0_hunt[1,ng_end[a],ng_end[t]]) + log(S0[1,yr_end[a-1],yr_end[t-1]]) - log(S0_hunt[1,yr_end[a-1],yr_end[t-1]]))/S0[1,yr_end[a-1],yr_end[t-1]]
        }
        for(a in 3:n_agem) {
            #s_hunt[2, a, t] <- S0_hunt[2, ng_end[a], ng_end[t]]/
            #                    S0[2, yr_end[a-1], yr_end[t - 1]]
            s_hunt[2,a,t] <- exp(log(S0_hunt[2,ng_end[a],ng_end[t]]) + log(S0[2,yr_end[a-1],yr_end[t-1]]) - log(S0_hunt[2,yr_end[a-1],yr_end[t-1]]))/S0[2,yr_end[a-1],yr_end[t-1]]
       }
    }
  returnType(double(3))
