...
calc_surv_aah <- nimble::nimbleFunction(
	...
	S0_fawn <- nimArray(NA,c(2,nT_age,nT_period))
    UCH <- nimArray(NA,c(2,nT_age,nT_period))
    UCH_fawn <- nimArray(NA,c(2,yr_step_intvl,nT_period))
...
#(endtime <- Sys.time() - starttime)
  for(i in 1:yr_step_intvl) {
        for(j in 1:nT_period) {
            UCH_fawn[1, i, j] <- exp(beta0 + beta_sex + age_effect[i] + period_effect[j])
            UCH_fawn[2, i, j] <- exp(beta0 + age_effect[i] + period_effect[j])
            S0_fawn[1, i, j] <- exp(-sum(UCH_fawn[1,1:i,yr_start[j]:yr_end[j]]))
            S0_fawn[2, i, j] <- exp(-sum(UCH_fawn[2,1:i,yr_start[j]:yr_end[j]]))
        }
		#print(c(i,j))
    }
#(endtime <- Sys.time() - starttime)
    ...
    for (t in 1:n_year) {
        s_aah[1, 1, t] <- S0_fawn[1, intvl_step_yr, yr_end_indx[t]] #antlerless fawns
        s_aah[2, 1, t] <- S0_fawn[2, intvl_step_yr, yr_end_indx[t]] #antlered fawns
    }
    ...
    for(a in 2:n_agef) {
        ...
        s_aah[1, a, 1] <- S0[1, a * intvl_step_yr, yr_end_indx[1]]
    }
    #antlered yearlings and older first year
    for(a in 2:n_agem) {
        #!#! s_aah[1, a, 1] <- S0[1, yr_end_indx[1], a * intvl_step_yr]
        s_aah[2, a, 1] <- S0[2, a * intvl_step_yr, yr_end_indx[1]]
    }
    ...
    for (t in 2:n_year) {
        for (a in 2:n_agef) {
            #s_hunt[1, a, t] <- S0_hunt[1, ng_end[a], ng_end[t]]/
             #                   S0[1, (a - 1) * intvl_step_yr, yr_end_indx[t - 1]]
            s_hunt[1, a, t] <- S0_hunt[1, ng_end[a], ng_end[t]]/
                                S0[1, yr_end_indx[a-1], yr_end_indx[t - 1]]
        }
        for(a in 2:n_agem) {
